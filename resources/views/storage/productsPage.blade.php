                ...
                <form id="productForm" method="post" action="/storage/addProduct">
                    ...
                    <div class="row">
                        <div class="col-md-5">
                            ...
                            <input class="form-control border-dark" type="text" id="productName" name="productName"
                                   autocomplete="off" value="{{old("productName")}}" required>
                        </div>
                        <div class="col-md-4">
                            <label class="form-label" for="productShortName">Rövid név:</label>
                            <input class="form-control border-dark" type="text" id="productShortName"
                                   name="productShortName" value="{{old("productShortName")}}" autocomplete="off" required>
                        </div>
                        <div class="col-md-3">
                            <label class="form-label" for="nPrice">Nettó ár:</label>
                            <input class="form-control border-dark" type="number" id="nPrice" name="nPrice"
                                   autocomplete="off" value="{{old("nPrice")}}" required>
                        </div>
                        <div class="col-md-5">
                            <div class="row">
                                <div class="col-md-10">
                                    <label class="form-label" for="categoryId">Kategória:</label>
                                    <select class="form-select border-dark" name="categoryId" id="categoryId"
                                            autocomplete="off" {{old("newCategoryName") != null ? 'disabled' : ''}} required>
                                        <option value="">...</option>
                                ...
                                <div class="col-md-2">
                                    <br>
                                    <button id="newCategoryButton" class="btn button-blue mt-2" type="button"
                                            data-bs-toggle="collapse" data-bs-target="#newCategoryNameInput"
                                            aria-expanded="false" aria-controls="newCategoryNameInput"
                                            {{old("categoryId") != null ? 'disabled' : ''}}>
                                            +
                            </button>
                        </div>
                        <div class="col-md-12 collapse {{old("newCategoryName") != null ? 'show' : ''}}" id="newCategoryNameInput">
                                    <label class="form-label" for="newCategoryName">Új kategória neve:</label>
                                    <input id="newCategoryName" class="form-control border-dark" type="text"
                                           name="newCategoryName" value="{{old("newCategoryName")}}">
                                </div>
                            </div>
                        </div>
                        <div class="col-md-4">
                            <label class="form-label" for="companyId">Beszállító:</label>
                            <select class="form-select border-dark" name="companyId" id="companyId" autocomplete="off">
                                <option value="">...</option>
                                @foreach(\App\Models\Company::all()->where('isSupplier', '=', true) as $supplier)
                                    <option value="{{$supplier->companyId}}" {{old("companyId") == $supplier->companyId ? 'selected' : ''}}>{{$supplier->companyName}}</option>
                                @endforeach
                            </select>
                        </div>
                        <div class="col-md-3">
                            <label class="form-label" for="bPrice">Bruttó ár:</label>
                            <input class="form-control border-dark" type="number" id="bPrice" name="bPrice"
                                   autocomplete="off" value="{{old("bPrice")}}">
                        </div>
                        <div class="d-flex pt-2 justify-content-center">
                            <button type="button" class="btn button-red"
                                    onclick="window.location.href = '/storage/menu'" style="margin: 0">Vissza a menübe
                            </button>
                            <input class="btn button-blue mx-2" type="submit" value="Mentés" style="margin: 0">
                            <button type="button" class="btn button-orange" onclick="emptyForm()" style="margin: 0">Új
                                felvétele
                            </button>
                        </div>
                    </div>
